...
This package expresses **Claude Design** as a typed, task-adaptive agent harness. Its concrete capability remains: Design one-off HTML artifacts such as landing pages, decks, and prototypes. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Claude Design |
| --- | --- | --- |
| Memory | `artifact_revision_history` | Retain design brief plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `iterative_composition_plan` | Transform content hierarchy into the ordered, bounded stages defined by this skill. |
...
| Action | `brief_generate_render_inspect_export` | Advance the workflow through html and css composition while preserving stop conditions and user authority. |
