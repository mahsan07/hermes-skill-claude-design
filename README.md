<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Claude Design" width="100%">
</p>
...
Creative requests often lose their intent between the initial brief, generation, export, and final quality check. **Claude Design** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `claude-design` names the capability directly—claude design—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Claude Design**.
...
| What does it do? | Design one-off HTML artifacts such as landing pages, decks, and prototypes. |
...
![Claude Design system map](assets/system-map.svg)

## Operation sequence

![Claude Design actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Claude Design](assets/example-output.svg)
...
> Using original or licensed sample material, design one-off HTML artifacts such as landing pages, decks, and prototypes. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
